...
if __name__ == "__main__":
    ...
    parser.add_argument("--sharp_name", type=str, default=r"sharp_data", help="Desired folder name for storing the sharp image files.")
    ...
    sharp_folder = os.path.join(base_folder, opt.sharp_name)
    if os.path.exists(sharp_folder):
        shutil.rmtree(sharp_folder)
        ...
        for i in trange(len(sub_files)):
            ...
            if i % (num_blur_inter) == num_blur_inter - 1:
                ...
                if end + opt.num_add >= len(sub_files):
                    break
                sharp_image = (imgs[len(imgs) // 2] * 255).astype(np.uint8)                
                # save
                out_file = sub_files[(start + end) // 2].replace(opt.img_type, ".png")
                cv2.imwrite(os.path.join(sharp_folder,out_file), sharp_image)
                print(f"Generating the sharp image {sub_files[(start + end) // 2]} with the exposure area ranges from {sub_files[start - opt.num_add]} to {sub_files[end + opt.num_add]}")
                imgs = []
                bais += opt.num_overlap
                start = i + bais + 1
